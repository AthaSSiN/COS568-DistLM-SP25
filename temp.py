...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            for name, param in model.named_parameters():
                if param.grad is not None:
                    if rank == 0:
                        gather_list = [torch.zeros_like(param.grad.data) for _ in range(world_size)]
                        ...
                        # Prepare list for scattering the average gradient
        ...
        if len(iteration_times) > 1:
            ...
            logger.info("Average iteration time for epoch (excluding first iteration): %.4f seconds", avg_time)
